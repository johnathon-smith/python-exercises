...
#importing sympy for bonus question
from sympy import *
...
number_of_fruit_chars = [len(fruit) for fruit in fruits]
print(number_of_fruit_chars)
...
numbers_with_at_least_two_digits = [number for number in numbers if abs(number) / 10 > 1]
print(numbers_with_at_least_two_digits)
...
#I found a helper function in the sympy library, so I will just use that
primes = [number for number in numbers if number > 0 and isprime(number)]
print(primes)